...
format:
  revealjs:
    theme: [default, serif, custom-presentation.scss]
    slide-number: true
    preview-links: auto
...
::: {.compact-list .incremental}

- **The Data**: 72K job postings → What story do they tell?
- **The Big 4 Factors**: Experience, Location, Industry, Remote Work
- **The Skill Gap**: What employers want vs. what we learn
- **The ROI Question**: Is a Master's degree worth $278K?
- **Your Action Plan**: Data-driven decisions you can make today
:::
...
# Ensure chart uses full width and auto-scales properly
fig.update_layout(
    autosize=True,
    width=None,
    height=500,
...
# Ensure chart uses full width and auto-scales properly for side-by-side layout
fig.update_layout(
    autosize=True,
    width=None,
    height=450,
...
- **63% premium** for top markets vs. median city
...
::: {.notes}
Geography is the second-biggest lever. Top markets pay 63% more than median cities. But the cost of living difference is real. Smart play: negotiate remote work from a lower-cost city—you keep the high salary AND the low expenses. This is the real arbitrage opportunity.
:::
...
Technology hubs pay **63% more** than median cities. But here's the strategic insight: with remote work, you can access these salaries without the cost of living.
...
#### Why This Works
  ...
  - Access high-paying markets remotely
  - Lower cost of living = higher real income
  - Best of both worlds: salary + lifestyle
:::
::::
...
- **Tech Services: $133K** | Finance: $114K | Healthcare: $90K | Education: $73K
...
# Ensure chart uses full width and auto-scales properly with reduced height
fig.update_layout(
    autosize=True,
    width=None,
    height=400,
...
# Display the math callout with calculated values
...
**Real cost: ${int(total_cost/1000)}K** (tuition $50K + foregone earnings ${int(foregone_earnings/1000)}K). Salary premium: **20%** (${int(annual_premium/1000)}K/year). **Breakeven: {breakeven} years**.
:::
"""
