...
def load_job_data():
    ...
    try:
        ...
        if len(df) == 0:
            print("⚠️ Processed data file is empty, falling back to raw data")
            raise FileNotFoundError("Empty processed file")
        print(f"✅ Loaded {len(df):,} job postings from processed dataset")
        return df
    except (FileNotFoundError, pd.errors.EmptyDataError):
        try:
            df = pd.read_csv('data/processed/job_market_sample.csv')
            if len(df) == 0:
                print("⚠️ Sample data file is empty, falling back to raw data")
                raise FileNotFoundError("Empty sample file")
            print(f"✅ Using sample dataset: {len(df):,} job postings")
            return df
        except (FileNotFoundError, pd.errors.EmptyDataError):
            try:
                print("📁 Loading raw Lightcast data and creating basic sample...")
                ...
                print(f"✅ Using raw data sample: {len(df):,} job postings")
                return df
            except Exception as e:
                print(f"❌ All data loading failed, creating synthetic data: {e}")
                ...
                print(f"✅ Generated synthetic dataset: {len(df):,} job postings")
                return df
```
