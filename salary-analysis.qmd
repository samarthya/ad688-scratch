...
from src.utils.get_stats import JobMarketStatistics
from src.visualization import SalaryVisualizer, QuartoChartExporter

# Initialize data processor with proper error handling
data_processor = JobMarketDataProcessor()
...
try:
    # Try to load sample data first (faster for testing)
    df = data_processor.load_and_process_data('data/processed/job_market_sample.csv')
    print(f"SUCCESS: Loaded and processed {df.count():,} job postings from sample dataset")
except Exception as e:
    try:
        # Fallback to raw data if sample doesn't exist
        df = data_processor.load_and_process_data('data/raw/lightcast_job_postings.csv')
        print(f"SUCCESS: Loaded and processed {df.count():,} job postings from raw dataset")
    except Exception as e2:
        print(f"ERROR: Unable to load any dataset: {e2}")
        raise e2  # Show the actual error for student analysis report

# Initialize visualizer with loaded data (convert Spark DataFrame to Pandas)
...
try:
    ...
    exp_analysis = pd.DataFrame([
        {'Experience Level': level, 'Median Salary': data['salary'],
         'Salary Increase': f"{data['increase_pct']:.1f}%", 'Formatted Salary': f"${data['salary']:,.0f}"}
...
print(exp_analysis[['Experience Level', 'Median Salary', 'Salary Increase', 'Formatted Salary']].to_string(index=False))
...
try:
    ...
    print("## Education Premium Analysis (Real Dataset)")
    print(f"**Note**: Analyzing education requirements from actual job postings - {e}\n")

    # Analyze education requirements from our real dataset
    if 'education_required' in df.columns:
        # Group by education level and calculate salary statistics
        edu_real = df.groupby('education_required')['salary_avg'].agg([
            'count', 'median', 'mean'
        ]).round(0).reset_index()

        # Sort by median salary
        edu_real = edu_real.sort_values('median')

        # Calculate premium vs lowest education level
        base_salary = edu_real['median'].iloc[0] if len(edu_real) > 0 else 50000
        edu_real['premium_pct'] = ((edu_real['median'] - base_salary) / base_salary * 100).round(0)

        # Format for display
        edu_real['formatted_salary'] = edu_real['median'].apply(lambda x: f"${int(x):,}")
        edu_real['formatted_premium'] = edu_real['premium_pct'].apply(lambda x: f"{int(x)}%" if x >= 0 else f"{int(x)}%")

        print("| Education Level | Median Salary | Premium vs Base | Job Count |")
        print("|----------------|---------------|-----------------|-----------|")

        for _, row in edu_real.iterrows():
            edu_level = row['education_required'][:20] if len(str(row['education_required'])) > 20 else row['education_required']
            salary = row['formatted_salary']
            premium = row['formatted_premium']
            count = int(row['count'])
            print(f"| {edu_level} | {salary} | {premium} | {count} |")

        print(f"\n**Key Education ROI Insights (Real Data Analysis):**")
        print(f"- Analysis based on {edu_real['count'].sum():,} job postings with education requirements")
        print(f"- Education premium calculated from actual job market data using our processing pipeline")
    else:
        # Fallback: Basic salary tier analysis by education inference
        print("**Education analysis by salary tier (inferred from compensation levels):**")

        salary_tiers = [
            ("Entry Level (< $60k)", df[df['salary_avg'] < 60000]),
            ("Professional ($60k-$100k)", df[(df['salary_avg'] >= 60000) & (df['salary_avg'] < 100000)]),
            ("Senior Professional ($100k-$150k)", df[(df['salary_avg'] >= 100000) & (df['salary_avg'] < 150000)]),
            ("Advanced Professional ($150k+)", df[df['salary_avg'] >= 150000])
        ]

        print("| Compensation Tier | Count | Median | Likely Education |")
        print("|------------------|-------|--------|------------------|")

        for tier_name, tier_data in salary_tiers:
            if len(tier_data) > 0:
                count = len(tier_data)
                median = tier_data['salary_avg'].median()
                if "Entry" in tier_name:
                    education = "High School - Associates"
                elif "Professional" in tier_name and "$60k" in tier_name:
                    education = "Bachelor's Degree"
                elif "Senior" in tier_name:
                    education = "Bachelor's + Experience"
                else:
                    education = "Advanced Degree"

                print(f"| {tier_name} | {count:,} | ${median:,.0f} | {education} |")

        print(f"\n**Key Education ROI Insights (Salary Tier Analysis):**")
        print(f"- Education requirements inferred from {len(df):,} job salary distributions")
        print(f"- Higher education correlates with access to higher-paying positions")
```
...
**[→ View Geographic Analysis](salary-geographic.qmd){target="_blank"}** - Interactive geographic salary analysis
...
**[→ View Salary Overview](salary-overview.qmd){target="_blank"}** - Executive market overview and key insights

### Salary Insights

**[→ View Education & Skills Analysis](salary-education.qmd){target="_blank"}** - Skills premium and education ROI analysis

### Remote Work Trends

**[→ View Remote Work Analysis](remote-work.qmd){target="_blank"}** - Remote work impact on compensation

### Occupation Analysis

**[→ View Industry Analysis](salary-industry.qmd){target="_blank"}** - Occupation trends and industry comparisons
...
try:
    ...
    print(f"ERROR: Executive summary analysis failed: {e}")
    print("This error indicates a data quality issue that needs to be addressed for the analysis to work properly.")
    raise e  # Re-raise the error so students can see what went wrong
```
...
try:
    ...
    print(f"ERROR: Strategic insights analysis failed: {e}")
    print("This error indicates a data quality issue that needs to be addressed for the analysis to work properly.")
    raise e  # Re-raise the error so students can see what went wrong
```
