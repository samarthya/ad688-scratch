...
try:
    # Try to load processed data first, automatically choosing the right processing method
    df = data_processor.load_and_process_data('data/processed/clean_job_data.csv', use_sample=False)
    print(f"SUCCESS: Loaded and processed {df.count():,} job postings from processed dataset")
except Exception as e:
    try:
        # Fallback to sample data with automatic sample processing
        df = data_processor.load_and_process_data('data/processed/job_market_sample.csv', use_sample=True)
        print(f"SUCCESS: Using sample dataset: {df.count():,} job postings")
