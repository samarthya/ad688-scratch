...
df_pandas = load_analysis_data("experience")
...
try:
    ...
    exp_analysis = pd.DataFrame([
        {'Experience Level': level, 'Median Salary': data['median'],
         'Salary Increase': f"{data.get('increase_pct', 0):.1f}%", 'Formatted Salary': f"${data['median']:,.0f}"}
...
try:
    ...
    for level, data in exp_progression.items():
        # Handle both 'median' and 'salary' keys for compatibility
        salary = int(data.get('median', data.get('salary', 0)))
...
if 'sector' in df_pandas.columns or 'industry' in df_pandas.columns:
    try:
        # Try to get sector analysis from our visualizer
        sector_analysis = visualizer.get_industry_salary_analysis(top_n=5)

        tech_median = df_pandas[df_pandas['industry'].str.contains('Tech|tech', na=False)]['SALARY_AVG'].fillna(0).median() if 'industry' in df_pandas.columns else 0
        non_tech_median = df_pandas[~df_pandas['industry'].str.contains('Tech|tech', na=False)]['SALARY_AVG'].fillna(0).median() if 'industry' in df_pandas.columns else 0
        ...
        print(f"- **{len(df_pandas):,}** positions analyzed from our enhanced processor")

    except:
        print(f"- Comprehensive analysis completed on **{len(df_pandas):,}** job market positions")
        print(f"- Career progression patterns identified using our data processing pipeline")
else:
    print(f"- Total positions analyzed: **{len(df_pandas):,}** using our JobMarketDataProcessor")
