...
df_pandas = load_analysis_data("geographic")
...
try:
    ...
    location_columns = [col for col in df_pandas.columns if any(geo in col.lower() for geo in ['location', 'city', 'state', 'region', 'area'])]
    ...
    if location_columns:
        ...
        geo_analysis = df_pandas.groupby(location_col)['SALARY_AVG'].agg([
        ...
        raise Exception("No geographic columns found - data processing issue")
...
try:
    ...
    remote_columns = [col for col in df_pandas.columns if any(remote in col.lower() for remote in ['remote', 'work_from_home', 'wfh', 'flexible'])]

    if remote_columns:
        remote_col = remote_columns[0]
        print(f"## Remote Work Analysis (Column: {remote_col})")

        remote_analysis = df_pandas.groupby(remote_col)['SALARY_AVG'].agg([
            'count', 'median', 'mean'
        ]).round(0).reset_index()
...
try:
    ...
    print(f"ERROR: Cost of living analysis failed: {e}")
    print("This error indicates a data quality issue that needs to be addressed for the analysis to work properly.")
    raise e  # Re-raise the error so students can see what went wrong
```
