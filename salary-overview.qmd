...
df_pandas = load_analysis_data("overview")
...
try:
    ...
    # Format for display
    overall_stats = {
        'median': f"${raw_stats['salary_median']:,.0f}",
        'mean': f"${raw_stats['salary_mean']:,.0f}",
        'std': f"${raw_stats['salary_std']:,.0f}",
        'range_min': f"${raw_stats['salary_min']:,.0f}",
        'range_max': f"${raw_stats['salary_max']:,.0f}",
        'total_jobs': raw_stats['total_jobs']
    }

    # Create comprehensive salary distribution chart
    salary_chart = chart_components.create_salary_distribution_chart(
        df_pandas,
        title="Comprehensive Salary Analysis Overview"
    )

    # Create summary statistics cards
    summary_cards = chart_components.create_summary_stats_cards(summary)
...
```{python}
#| label: salary-visualization
#| echo: false
#| output: true

# Display the comprehensive salary distribution chart
import plotly.io as pio
pio.show(salary_chart)
```
