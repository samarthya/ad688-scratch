...
try:
    ...
    print(f"Loaded and processed {df.count():,} job postings from sample dataset for statistical analysis")
    ...
    try:
        ...
        print(f"Loaded and processed {df.count():,} job postings from raw dataset for statistical analysis")
    except Exception as e2:
        print(f"ERROR: Unable to load any dataset: {e2}")
        raise e2  # Show the actual error for student analysis report

# Initialize our analysis classes (convert Spark DataFrame to Pandas)
df_pandas = df.toPandas()  # Convert to Pandas for analysis
...
# Calculate comprehensive salary statistics directly
try:
    # Clean and prepare salary data
    # Convert to numeric, coercing errors to NaN
    salary_data = pd.to_numeric(df_pandas['SALARY_AVG'], errors='coerce').dropna()

    print("## Salary Distribution Statistics (Direct Calculation)")
    print(f"**Dataset**: {len(df_pandas):,} job postings")
    print(f"**Valid salary records**: {len(salary_data):,} job postings")
    print()
    ...
    salary_stats = {
        ...
        'skewness': scipy_stats.skew(salary_data),
        'kurtosis': scipy_stats.kurtosis(salary_data)
    }
...
print(f"\n**Analysis based on {len(df_pandas):,} job postings**")
```
...
print(f"\n**Statistical modeling using scikit-learn on {len(df_pandas):,} job postings**")
```
...
print(f"\n**Distribution analysis completed on {len(df_pandas):,} salary observations**")
```
...
print(f"\n**Advanced modeling using ensemble methods on {len(df_pandas):,} observations**")
```
...
print(f"\n**Comprehensive statistical analysis based on {len(df_pandas):,} job market observations**")
```
