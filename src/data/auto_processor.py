...
def load_analysis_data(analysis_type="comprehensive"):
    ...
    """
    logger.info(f"Loading data for {analysis_type} analysis...")
    ...
    logger.info(f" [OK] Loaded {len(df):,} records with standardized columns")

    return df

def process_raw_data(df):
    """Process raw Lightcast data using centralized column mapping."""
    logger.info(" Processing raw Lightcast data...")
    ...
    if 'CITY_NAME' in df.columns and 'CITY' in df.columns:
        logger.info(" Found both CITY_NAME and CITY - prioritizing CITY_NAME (plain text)")
        ...
        logger.info(" Using CITY_NAME as location source")
        df = df.rename(columns={'CITY_NAME': 'city_name'}) # Convert to snake_case
    elif 'CITY' in df.columns:
        logger.info(" Using CITY as location source (will need decoding)")
    ...
    if mapping_to_apply:
        logger.info(f" Applying {len(mapping_to_apply)} column mappings")
        df = df.rename(columns=mapping_to_apply)

    # Standardize ALL remaining UPPERCASE columns to snake_case
    logger.info(" Standardizing all remaining UPPERCASE columns to snake_case...")
    ...
    if uppercase_columns:
        ...
        logger.info(f" Converting {len(snake_case_mapping)} UPPERCASE columns to snake_case")
        df = df.rename(columns=snake_case_mapping)

    # Ensure city_name column always exists for geographic analysis
    if 'city_name' not in df.columns:
        if 'location' in df.columns:
            logger.info(" Creating city_name from location column")
            df['city_name'] = df['location']
        else:
            logger.info(" Creating default city_name column")
    ...
    for col in salary_raw_cols:
        if col in df.columns:
            ...
            if before_convert != after_convert:
                logger.info(f" Converted {col}: {before_convert:,} → {after_convert:,} valid values")

    # Check if salary_avg already exists (from clean sample data)
    salary_avg_exists = False
    if 'salary_avg' in df.columns:
        valid_count = df['salary_avg'].notna().sum()
        logger.info(f" Found existing salary_avg column: {valid_count:,}/{len(df):,} valid ({valid_count/len(df)*100:.1f}%)")

        if valid_count > len(df) * 0.5:
            # Already has computed salary_avg with good coverage, use it
            logger.info(f" [OK] Using existing salary_avg column - skipping computation")
            salary_avg_exists = True
        else:
            logger.warning(f" [WARNING] Existing salary_avg has low coverage, will compute from salary components")
    ...
    if not salary_avg_exists and ('salary_single' in df.columns or 'salary' in df.columns):
        ...
        if 'salary_min' in df.columns and 'salary_max' in df.columns:
            ...
            if fill_mask.sum() > 0:
                ...
                logger.info(f" Computed {fill_mask.sum():,} salary_avg values from salary ranges")
        ...
        logger.info(f" Computed {both_exist.sum():,} salary_avg values from salary ranges")

    elif not salary_avg_exists:
        # Create synthetic salary data for testing
        logger.info(" No salary data found - creating synthetic data for testing")
    ...
    if 'salary_avg' in df.columns:
        ...
        if invalid_count > 0:
            logger.info(f" [CLEAN] Marking {invalid_count:,} unrealistic salary values as missing for imputation")
    ...
    if missing_salary_mask.sum() > 0:
        logger.info(f" [TIP] Imputing {missing_salary_mask.sum():,} missing salary values using industry grouping...")
            ...
            for industry, median_salary in industry_medians.items():
                if pd.notna(median_salary):
                    ...
                    if mask.sum() > 0:
                        ...
                        logger.info(f" Imputed {mask.sum():,} salaries for {industry}: ${median_salary:,.0f}")
        ...
        if missing_salary_mask.sum() > 0:
            ...
            if pd.notna(overall_median):
                ...
                logger.info(f" Imputed {missing_salary_mask.sum():,} remaining salaries with overall median: ${overall_median:,.0f}")
            else:
                # Ultimate fallback
                df.loc[missing_salary_mask, 'salary_avg_imputed'] = 75000
                logger.info(f" Applied fallback salary of $75,000 to {missing_salary_mask.sum():,} records")

    # Final data quality assurance for analysis-ready data
    logger.info(" [CHECK] Final data quality checks...")
    ...
    if invalid_count > 0:
        logger.info(f" Removing {invalid_count:,} records with invalid salary data")
    ...
    if final_invalid_count > 0:
        logger.info(f" Removing {final_invalid_count:,} records with unrealistic salary values")
        df = df[final_valid_mask].copy()

    logger.info(f" [OK] Final dataset: {len(df):,} records with clean salary data")

    # Standardize experience columns and ensure they are numeric
    logger.info(" [DATA] Processing experience data...")
    ...
    if 'experience_min' in df.columns and 'experience_max' in df.columns:
        ...
        if swap_mask.any():
            logger.info(f" Swapping {swap_mask.sum()} records where min > max experience")
    ...
    logger.info(" [COMPUTE] Creating derived numeric columns...")
    ...
    logger.info(f" [OK] Experience data processing completed")
...
def process_processed_data(df):
    ...
    logger.info(" Processing pre-processed data...")
...
def validate_data_for_analysis(df, analysis_type="comprehensive"):
    ...
    if missing:
        logger.warning(f"[WARNING] Missing columns for {analysis_type} analysis: {missing}")
        return False
