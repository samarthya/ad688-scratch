...
class JobMarketStatistics:
    ...
    def load_data(self, file_path: Optional[str] = None) -> pd.DataFrame:
        ...
        try:
            ...
            salary_cols = ['SALARY_FROM', 'SALARY_TO', 'SALARY_AVG']
    ...
    def calculate_salary_statistics(self) -> Dict[str, Any]:
        ...
        # Use SALARY_AVG as primary metric, fallback to SALARY_FROM or SALARY_TO
        print(f"Available columns: {list(self.df.columns)}")
        if 'SALARY_AVG' in self.df.columns:
            salary_col = 'SALARY_AVG'
            print(f"Using SALARY_AVG column")
        elif 'SALARY_FROM' in self.df.columns:
            salary_col = 'SALARY_FROM'
            print(f"Using SALARY_FROM column")
        elif 'SALARY_TO' in self.df.columns:
            salary_col = 'SALARY_TO'
            print(f"Using SALARY_TO column")
        elif 'Maximum Annual Salary' in self.df.columns:
            salary_col = 'Maximum Annual Salary'
            print(f"Using Maximum Annual Salary column")
        else:
            salary_col = 'Minimum Annual Salary'
            print(f"Using Minimum Annual Salary column (fallback)")
        ...
        stats = {
            ...
            'percentiles': {
                '25th': salary_data.quantile(0.25),
                '75th': salary_data.quantile(0.75),
                '90th': salary_data.quantile(0.90),
                '95th': salary_data.quantile(0.95)
            }
        }
        ...
        stats['skewness'] = scipy_stats.skew(salary_data)
        stats['kurtosis'] = scipy_stats.kurtosis(salary_data)
