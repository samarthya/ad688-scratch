...
```{python}
#| label: setup-format-detection
#| include: false

# Detect output format for conditional rendering
import sys
import os
from pathlib import Path

# Detect if rendering to DOCX by checking Quarto's format metadata
is_docx = False
try:
    # Check for QUARTO_FORMAT environment variable (Quarto sets this)
    quarto_format = os.environ.get('QUARTO_FORMAT', '')
    if 'docx' in quarto_format.lower():
        is_docx = True
    # Also check command line args
    elif any('docx' in str(arg).lower() for arg in sys.argv):
        is_docx = True
    # Check if in docx-specific context
    elif hasattr(__builtins__, 'display'):
        # In Jupyter/interactive context - use HTML
        is_docx = False
except:
    is_docx = False

# Helper function to display figures based on format
def display_figure(fig, filename=None):
    """Display figure appropriately based on output format"""
    # Always try to save as PNG for DOCX compatibility
    if filename:
        png_path = f"figures/{filename}.png"
        Path("figures").mkdir(exist_ok=True)
        try:
            # Save static PNG (works for both HTML and DOCX)
            fig.write_image(png_path, width=1200, height=800, scale=2)
        except Exception as e:
            pass  # Silently fail if kaleido not available

    # Always show interactive for rendering
    fig.show()

print(f"Format detection: DOCX mode = {is_docx}")
print(f"Figures will be saved to figures/ directory for DOCX compatibility")
```
